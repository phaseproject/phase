...
#if defined(NDEBUG)
# error "Phase Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    CAmount nSubsidy;
    if((nPrevHeight+1) < founderPayment.getFounderStartHeight()) {
    	nSubsidy = 50 * COIN;
    } else {
    	nSubsidy = 55 * COIN;
    }

    //  decline of production by 30% per 90k.
    for (int i = consensusParams.nFirstSubsidyReduceBlock; i <= nPrevHeight; i += consensusParams.nSubsidyHalvingInterval) {
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // PHASE : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(PHASE): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(PHASE): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // PHASE : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(PHASE): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(PHASE): spork is off, skipping transaction locking checks\n");
    }
    ...
    if(!founderTransaction) {
		...
		return state.DoS(0, error("CheckBlock(PHASE): transaction %s does not contains founder transaction",
