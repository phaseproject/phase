...
const char * const BITCOIN_CONF_FILENAME = "phase.conf";
const char * const BITCOIN_PID_FILENAME = "phased.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "phase" is a composite category enabling all Proton-related debug output
            if(ptrCategory->count(string("phase"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PhaseCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PhaseCore
    // Mac: ~/Library/Application Support/PhaseCore
    // Unix: ~/.phasecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PhaseCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PhaseCore";
#else
    // Unix
    return pathRet / ".phasecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty phase.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override phase.conf
