...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        if(!isValidMasternode(coins.vout[vin.prevout.n].nValue, height)) {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have at least %"PRId64" PHASE, masternode=%s\n", getMinimumCollateral(height), vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 5000 PHASE tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 5000 PHASE tx -> 1 confirmation
