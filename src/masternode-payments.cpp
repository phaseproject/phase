...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f PHASE\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
...
bool CMasternodePaymentVote::IsValid(CNode* pnode, int nValidationHeight, std::string& strError)
{
    ...
    if(nRank > MNPAYMENTS_SIGNATURES_TOTAL) {
        ...
        if(nRank > MNPAYMENTS_SIGNATURES_TOTAL*2 && nBlockHeight > nValidationHeight) {
            ...
            //Misbehaving(pnode->GetId(), 20); // comment out for now
        }
